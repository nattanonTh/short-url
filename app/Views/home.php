...
<section>
	<div>
		<form method="post" id="create-short-url" autocomplete="off" action="<?= base_url('short-url') ?>">
			<input type="text" class="form-control" id="original_url" name="original_url">
			<button type="submit" class="btn my-3">Generate</button>
		</form>
	</div>
		...
		<table class="table table-bordered">
			...
			<?php if($shortUrls): ?>
				<?php foreach($shortUrls as $shortUrl): ?>
					<tr>
						<td><?php echo $shortUrl['original_url']; ?></td>
						<td>
							<a href="<?php echo $shortUrl['short_url']; ?>" class="btn btn-primary btn-sm">
								<?php echo $shortUrl['short_url']; ?>
							</a>
						</td>
					</tr>
				<?php endforeach; ?>
